...
public class Konfigurator {
	...
	private void generateObjectsFromJSON(String json) {
		...
		for (int i = 0; i < arr.length(); ++i) {
			Modell modell = new Modell(arr.getJSONObject(i).getString("name"), arr.getJSONObject(i).getDouble("preis"), arr.getJSONObject(i).getInt("lieferzeit"));

			JSONArray arrpak = arr.getJSONObject(i).getJSONArray("pakete");

			for (int x = 0; x < arrpak.length(); x++) {
				Paket paket = new Paket(arrpak.getJSONObject(x).getString("name"), arrpak.getJSONObject(x).getString("beschreibung"), arrpak.getJSONObject(x).getDouble("preis"));
	...
	private String dateiInhalt() {
		...
		try {
			fr = new FileReader("/Users/ahmetulas/Documents/workspace/Carconfig/src/carconfig/Daten/file.json");
